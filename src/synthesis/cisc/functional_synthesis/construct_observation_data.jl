...
function generate_observations_space_invaders(m::Module)
  ...
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/deterministic_space_invaders_output_2.jld", "chase")
...
function generate_observations_magnets(m::Module)
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/magnets_final.jld", "data") # JLD.load("magnets_i_observations.jld")["observations"]
...
function generate_observations_particles(m::Module)
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/deterministic_particles_input.jld", "particles")
  # state = Base.invokelatest(m.init, nothing, nothing, nothing, nothing, nothing)
  # observations = []
  # user_events = []
  # push!(observations, Base.invokelatest(m.render, state.scene))
...
function generate_observations_chase(m::Module) 
  JLD.load("test/cisc/data/observed/deterministic_chase_output_4.jld")["data"]
  # state = Base.invokelatest(m.init, nothing, nothing, nothing, nothing, nothing)
  # observations = []
  # user_events = []
  # push!(observations, Base.invokelatest(m.render, state.scene))
...
function generate_observations_ants(m::Module)
  JLD.load("test/cisc/data/observed/deterministic_ants_input.jld", "ants")
  # state = Base.invokelatest(m.init, nothing, nothing, nothing, nothing, nothing)
  # observations = []
  # user_events = []
  # push!(observations, Base.invokelatest(m.render, state.scene))
...
function generate_observations_sokoban(m::Module)
  ...
  # end
  # observations, user_events, 16
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/final_sokoban_observations.jld", "data")
  observations = observations[1]
  user_events = user_events[1]
...
function generate_observations_coins(m)
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/coins_full_reverse.jld", "data")
  observations = observations[1]
  user_events = user_events[1]
...
function generate_observations_coins5(m)
  # 5 coins
  JLD.load("test/cisc/data/observed/new_coins_data_5.jld")["data"]
end

function generate_observations_coins7(m)
  # 7 coins 
  JLD.load("test/cisc/data/observed/new_coins_data_7.jld")["data"]
end

function generate_observations_coins9(m)
  # 9 coins?
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/new_coins_data_9.jld")["data"]
...
function generate_observations_grow2(m)
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/observations_grow_ii.jld")["observations"]
  observations, user_events, grid_size
end

function generate_observations_mario2(m)
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/observations_mario_ii.jld")["observations"]
...
function generate_observations_swap(m::Module)
  observations, user_events, grid_size = JLD.load("test/cisc/data/observed/observations_swap_FINAL.jld")["observations"]
  observations, user_events, grid_size
  # state = Base.invokelatest(m.init, nothing, nothing, nothing, nothing, nothing)
  # observations = []
  # user_events = []
